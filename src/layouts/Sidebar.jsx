...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full md:min-h-screen bg-gradient-to-b from-yellow-600 to-yellow-800 p-2 z-50
      ...
      >
        ...
        <nav className="flex flex-col gap-3" role="menu">
          {sidebarLinks.map(({ path, label, icon, children }) =>
            !children ? (
              <NavLink
                key={path}
                to={`/admin${path ? "/" + path : ""}`}
              ...
              <div key={label} className="relative group">
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-4 py-3 rounded-lg font-semibold text-yellow-100 hover:bg-yellow-500 hover:text-neutral-900 transition-colors`}
                >
                  <span className="w-6 h-6 flex-shrink-0 text-inherit">{icon}</span>
                  {(desktopOpen || mobileOpen) && (
                    <>
                ...
                {/* Dropdown Children */}
                {desktopOpen && openDropdown === label && (
                  <div
                    className="ml-6 mt-2 overflow-hidden rounded-xl bg-yellow-700/70 shadow-md transition-all duration-300"
                  >
                    <div className="flex flex-col gap-1 p-2">
                      {children.map((child) => (
                        <NavLink
                          ...
                          to={`/admin/${child.path}`}
                          className={({ isActive }) =>
                            `flex items-center gap-3 rounded-lg px-4 py-2 text-sm font-medium transition-colors font-semibold
                            ${
                              isActive
                                ? "bg-yellow-500 text-neutral-900"
                                : "text-yellow-100 hover:bg-yellow-400 hover:text-neutral-900"
                            }`
                          }
                          onClick={() => setMobileOpen(false)}
                        >
                          <span className="w-2 h-2 rounded-full bg-yellow-100" />
